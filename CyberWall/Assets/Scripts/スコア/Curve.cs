...
public class Curve : MonoBehaviour
{
	...
	void Update () {
        if (EnergySlider.Instance.hp > 0)
        {
            ...
            controller._V_CW_Bend_Y += one * Time.deltaTime * 0.23f*PlayerManager.Instance.GetAccelSpeed()/PlayerManager.Instance.PlayerParam.MaxSpeed;
            if (Mathf.Abs(controller._V_CW_Bend_X) > 1)
            {
                controller._V_CW_Bend_X = 1 * Mathf.Sign(controller._V_CW_Bend_X);
                one2 *= -1;
            }
            controller._V_CW_Bend_X += one2 * Time.deltaTime * 0.2f * PlayerManager.Instance.GetAccelSpeed() / PlayerManager.Instance.PlayerParam.MaxSpeed;
        }
    }
}
